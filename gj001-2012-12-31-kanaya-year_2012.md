...
connector which includes 2 power lines (Vcc and GND), a reset pin, USB
Data+ and Data- lines, serial TX and RX lines, I2C (TWI) lines, and
SPI lines. The serial (2 lines), I2C (2 lines), SPI (3 lines) can be
bound to 7 bit lane and drive LCD device outside the box
(theoretically).
...
# 3.3 StepStone Computer I

Inspired from the successful design of _Pineapple II_, which will be
completely open hardware, I'm finishing design of a new microcomputer
based on mbed technology. This new computer will be far easier to use
than _Pineapple II_. The name of the compule will be, but not fixed
to, _StepStone Computer I_.
